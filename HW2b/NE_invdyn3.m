...
timerVal1 = tic;
    ...
    [m,ri_h_ci,ri_i_ci,It] = DynParameters(v,n);
...
%     n = Tsize(3);
    ...
    for i = 2:n
        b_sym(:,:,i) = transpose(R0_i_sym(:,:,i))*z_h(:,:,1);
    end
...
%     end
    ...
    for t = t_0:t_end/t_stp
        ...
        for i = n:-1:1
            ...
            norm(:,:,i)	= Rh_i(:,:,i+1)*norm(:,:,i+1)-cross(f(:,:,i),ri_h_ci(:,i))+cross(Rh_i(:,:,i+1)*f(:,:,i+1),ri_i_ci(:,i))+cross(omega_h(:,:,i+1),(It(:,:,i)*omega_h(:,:,i+1)))+It(:,:,i)*alpha_h(:,:,i+1);
