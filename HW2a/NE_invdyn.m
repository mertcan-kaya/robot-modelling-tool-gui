...
%     end
    ...
    omega_h_sym     = zeros(3,1,n+1,'sym');
    omega_d_h_sym	= zeros(3,1,n+1,'sym');
    ...
    for t = t_0:t_end/t_stp
        ...
        for i = 1:n
            omega_h_sym(:,:,i+1)  	= transpose(Rh_i(:,:,i))*omega_h_sym(:,:,i)+b(:,:,i)*q_d_sym(i);
            omega_d_h_sym(:,:,i+1)	= diff(omega_h_sym(:,:,i+1),time);

            omega_h(:,:,i+1)       	= subs(omega_h_sym(:,:,i+1),time,t);
            omega_d_h(:,:,i+1)    	= subs(omega_d_h_sym(:,:,i+1),time,t);
